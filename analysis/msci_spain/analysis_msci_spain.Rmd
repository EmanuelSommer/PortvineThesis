...
missl_resid_plots <- marg_resid_viz_list(missl_risk_roll)
missl_squaredresid_plots <- marg_resid_viz_list(missl_risk_roll, squared = TRUE)
```
...
```{r}
# all assets simultainiously
ljung_heatmap(missl_risk_roll)
ljung_heatmap(missl_risk_roll, 3)

# an animation over all marginal windows:
ljung_heatmap_animation(missl_risk_roll)
```
...
risk_estimates(missl_risk_roll,
  ...
  ggplot() +
  geom_line(aes(x = row_num, y = realized), col = "lightgrey") +
  geom_line(aes(x = row_num, y = risk_est), col = custom_colors[1]) +
  geom_point(aes(x = row_num, y = realized), col = custom_colors[2], 
...
Use the `esback` package for backtesting.

```{r}
# use the esback package for ES backtesting
esback::er_backtest(
  r = risk_estimates(missl_risk_roll, alpha = 0.05) %>%
    filter(risk_measure == "VaR") %>%
    pull(realized),
  q = risk_estimates(missl_risk_roll, alpha = 0.05) %>%
    filter(risk_measure == "VaR") %>%
    pull(risk_est),
  e = risk_estimates(missl_risk_roll, alpha = 0.05) %>%
    filter(risk_measure == "ES_mean") %>%
    pull(risk_est)
)[1:2]
esback::cc_backtest(
  r = risk_estimates(missl_risk_roll, alpha = 0.05) %>%
...
# visualize the exceedance residuals tested with er_backtest
risk_estimates(missl_risk_roll, alpha = 0.05, exceeded = TRUE) %>%
  inner_join(risk_estimates(missl_risk_roll, alpha = 0.05, exceeded = TRUE) %>%
               filter(exceeded & risk_measure == "VaR") %>%
               select(row_num),
             by = "row_num") %>%
  filter(risk_measure %in% c("ES_mean")) %>%
  mutate(exceed_resid = realized - risk_est) %>%
  ggplot(aes(x = exceed_resid)) + 
  geom_histogram(bins = 40, alpha = 0.3) +
  labs(x = "Exceedance residuals") +
  geom_density() +
  geom_vline(aes(xintercept = mean(exceed_resid)), col = custom_colors[1],
             size = 1)
```
...
risk_estimates(missl_cond_risk_roll, risk_measures = "ES_mean",
  ...
  geom_line(aes(x = row_num, y = iberdrola, col = factor(cond_u)),
            size = 0.5) +
  scale_color_manual(values = c(custom_colors[1], custom_colors[3]),
...
risk_estimates(missl_cond_risk_roll, risk_measures = "ES_mean",
  ...
  geom_line(
    ...
    aes(x = 1001:nrow(msci_spain_complete_data),
        y = iberdrola), col = "lightgrey", size = .3) +
  geom_line(aes(x = row_num, y = iberdrola),
            col = custom_colors[4],
            size = 0.5) +
  labs(title = "Conditional variable: Iberdrola",
       x = "estimation window", y = "log returns",
       subtitle = paste0("The conditional values corresponding to the residual
                     ...
                     ".")) +
  theme_light() +
  ggforce::facet_zoom(xlim = c(1300, 1330)) +
  theme(axis.text.x = element_blank(),
...
Nice to see so we expect the corresponding conditional risk measures with low conditional $\alpha$ level to be way more conservative under the assumption that Iberdrola has a generally positive dependence with the portfolio of interest. Moreover one can observe that the conditional values corresponding to the prior day residuals are closely following the observed path while lagging a little behind and thus exaggerating sudden drops or highs which might be a desirable behavior for risk estimation. Therefore look at the next plots.
...
risk_estimates(missl_cond_risk_roll, risk_measures = "ES_mean",
  ...
  geom_line(aes(x = row_num, y = realized), col = "lightgrey") +
  geom_line(aes(x = row_num, y = risk_est,
                col = factor(cond_u), linetype = factor(cond_u)),
            size = 0.5) +
  labs(x = "estimation window", y = "portfolio log returns",
       linetype = "Conditional alpha level",
       title = "Comparison of risk measure behavior for 2 conditional alpha levels") +
  scale_color_manual(values = c(custom_colors[1], custom_colors[3], "violet"),
                     name = "Conditional alpha level") +
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank())
...
risk_estimates(missl_cond_risk_roll, risk_measures = "ES_mean",
  ...
  geom_line(aes(x = row_num, y = risk_est),
            ...
            size = 0.5) +
  labs(x = "estimation window", y = "portfolio log returns",
       title = "Behavior of the conditional pior residual risk measure") +
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank())
```
...
risk_estimates(missl_cond_risk_roll, risk_measures = "ES_mean",
  ...
  labs(x = "estimation window",
       ...
       subtitle = paste0("Exceedances are highlighted in ",
                     ...
                     "'>**red**</span>",
                     "."),
       title = "Risk measure: ES (mean estimation) with alpha and prior residual conditional level"
       ) +
  theme(
    axis.text.x = element_blank(),
    axis.ticks.x = element_blank(),
...
head(risk_estimates(cond2test_risk_roll))
```
