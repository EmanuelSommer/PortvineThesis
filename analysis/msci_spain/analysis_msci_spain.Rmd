...
```{r es_ts_estimators_16, echo=FALSE}
risk_estimates(uncond_risk_roll_16_19_g50_k25_p250,
               risk_measures = c("ES_mean", "ES_median", "ES_mc"),
               alpha = 0.05) %>%
  left_join(msci_spain_16_19 %>%
            ...
            by = "row_num") %>%
  ggplot(aes(x = date, y = risk_est, col = risk_measure)) +
  geom_line() +
  scale_color_manual(values = c("orange", custom_colors[3:4]),
                     name = "Estimator",
                     label = c("Monte Carlo", "Mean", "Median")) +
  labs(title = "Comparison of Expected Shortfall estimatiors over time",
       x = "", y = "Estimated risk measure")
```
...
```{r es_ts_estimators_20, echo=FALSE}
risk_estimates(uncond_risk_roll_20_21_g50_k25_p200,
               risk_measures = c("ES_mean", "ES_median", "ES_mc"),
               alpha = 0.05) %>%
  ...
  scale_color_manual(values = c("orange", custom_colors[3:4]),
                     ...
                     label = c("Monte Carlo", "Mean", "Median")) +
  labs(title = "Comparison of Expected Shortfall estimatiors over time",
       x = "2021", y = "Estimated risk measure")
```
...
```{r cond1_sp500_16_realizedresid}
risk_estimates(cond_risk_roll_16_19_sp500, risk_measures = "ES_mean",
               cond_u = "resid",
               alpha = 0.05, exceeded = TRUE) %>%
  left_join(msci_spain_16_19 %>%
...
risk_estimates(cond_risk_roll_16_19_eurostoxx50_prior_resid, risk_measures = "ES_mean",
               cond_u = "prior_resid",
               alpha = 0.05, exceeded = TRUE) %>%
  left_join(msci_spain_16_19 %>%
              mutate(row_num = 1:nrow(msci_spain_16_19)),
...
risk_estimates(cond2_risk_roll_16_19, risk_measures = "ES_mean",
  ...
  labs(title = "Quantile based conditional Expected Shortfall", col = "Quantile level",
       x = "estimation window", y = "portfolio log returns",
       subtitle = "Alpha level: 2.5%<br>Conditional variables: Eurostoxx 50, Iberdrola")
```
