...
```{r, message=FALSE}
t(t(es_0.05_estimates) - true_es[3, ]) %>%
  as_tibble() %>%
...
t(t(es_0.1_estimates) - true_es[2, ]) %>%
  as_tibble() %>%
  pivot_longer(everything(), names_to = "day", values_to = "bias") %>%
  mutate(day = as.factor(as.integer(str_remove(day, "day_")))) %>%
  ggplot() +
...
t(t(es_0.2_estimates) - true_es[1, ]) %>%
  as_tibble() %>%
  pivot_longer(everything(), names_to = "day", values_to = "bias") %>%
  mutate(day = as.factor(as.integer(str_remove(day, "day_")))) %>%
  ggplot() +
...
We can observe that the estimation bias is almost always very close to 0 compared to the base variability of the log returns ($\hat\sigma = 0.083$). Moreover we can observe that the estimation quality does slightly degrade during this simulated forecasting window. This emphasizes the need to correctly calibrate the refitting frequency to the use case at hand. Furthermore we do not seem to systematically under or overestimate the ES especially across the first 10 days. This especially holds for the more realistic alpha levels like 0.05. This is good for both the regulators which are averse against systematic overestimation and the financial institutions which are avers against systematic underestimation as it means elevated capital requirements. This is however a proof of concept simulation study with clear limitations like only a small simulation number and thus especially the true ES that we are stating for comparison is only an estimation based on a rather small sample size. So all in all this PoC study did not uncover any obvious problems in the stated estimation approach and suggests that a large scale simulation study is the next step to take.
