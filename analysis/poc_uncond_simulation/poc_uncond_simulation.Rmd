...
We first want to simulate $n = 1100$ portfolios from this ground truth. We achieve this with the following steps:
...
In the following we will use a training horizon of $730$ days and a validation horizon of $20$ days. 
...
simulate_one_series <- function(ground_truth_portfolio, ground_truth_vine, marginals) {
  ...
  # estimate the ES at 10% for the simulated portfolio and record the ES estimates
  ...
  risk_estimates <- risk_estimates(risk_roll, alpha = c(0.05, 0.1))
...
realized_returns <- matrix(nrow = 1100, ncol = 20)
...
pb <- progress_bar$new(
  ...
  total = 1100, clear = FALSE
)
for (simulation_number in seq(1100)) {
  pb$tick()
  try({
  ...
  if ((simulation_number %% 50) == 0) {
    save(
      realized_returns, es_0.05_estimates, es_0.1_estimates,
      file = here::here("data", "poc_uncond_simulation_exp3.RData")
    )
  }
}
...
```{r}
load(here::here("data", "poc_uncond_simulation_exp3.RData"))
```

Estimate the *true* ES first.

```{r}
true_es <- vapply(1:20, function(day) {
  est_es(realized_returns[, day], alpha = c(0.1, 0.05))
}, FUN.VALUE = numeric(2))
rownames(true_es) <- c("alpha_0.1", "alpha_0.05")
true_es
```
...
bias_0.1 <- colMeans(es_0.1_estimates) - true_es[1, ]
tibble(bias = bias_0.1) %>%
  ggplot(aes(x = bias)) +
  geom_density() +
  geom_vline(xintercept = mean(bias_0.1), col = "blue") + 
  geom_vline(xintercept = 0)
bias_0.05 <- colMeans(es_0.05_estimates) - true_es[2, ]
...
```{r, message=FALSE}
t(t(es_0.05_estimates) - true_es[2, ]) %>%
  as_tibble() %>%
...
t(t(es_0.1_estimates) - true_es[1, ]) %>%
  as_tibble() %>%
  pivot_longer(everything(), names_to = "day", values_to = "bias") %>%
  mutate(day = as.factor(as.integer(str_remove(day, "day_")))) %>%
  ggplot() +
...
We can observe that the estimation bias is almost always very close to 0. Moreover we can observe that the estimation quality does not degrade during this simulated forecasting window. This is indeed an encouraging result. Furthermore we do not seem to systematically under or overestimate the ES across all the days. This is good for both the regulators which are averse against systematic overestimation and the financial institutions which are avers against systematic underestimation as it means elevated capital requirements. This is however a proof of concept simulation study with clear limitations like only a small simulation number and thus especially the true ES that we are stating for comparison is only an estimation based on a rather small sample size. So all in all this PoC study did not uncover any obvious problems in the stated estimation approach and suggests that a large scale simulation study is the next step to take.
